...
public class AtomDeserializer extends AbstractAtomDealer implements ODataDeserializer {
  ...
  private ODataError error(final XMLEventReader reader, final StartElement start) throws XMLStreamException {
    ...
    boolean setInnerError = false;
    boolean innerErrorSet = false;
    ...
    while (reader.hasNext() && !foundEndElement) {
      ...
      if (event.isStartElement()) {
        if (errorCodeQName.equals(event.asStartElement().getName())) {
          ...
          setTarget = true;
          //Vimal - have added the inner error details such that the error from SP is given back to the calling client program
          //this actually works in JSON format, ATOM did not code for inner error looks like...
        }else if(innerErrorQName.equals(event.asStartElement().getName())) {
        	setInnerError = true;
        }
      }
      ...
      if (event.isCharacters() && !event.asCharacters().isWhiteSpace()) {
        ...
        //Vimal - have added the inner error details such that the error from SP is given back to the calling client program
        //this actually works in JSON format, ATOM did not code for inner error looks like...
        if (setInnerError && !innerErrorSet) {
            HashMap<String, String> innerErrorMap = new HashMap<String, String>();
        	innerErrorMap.put("innerMessage", event.asCharacters().getData());
